  ...
  return (
    <header>
      ...
      <nav
        ...
        style={{ backgroundColor: "#005fb5" }}
      >
        <div className="container text-end">
          <div
            onClick={() => setAnimation(!fadeAnimation)}
            className="hamburger w-100 mb-3 d-block d-md-none"
          >
            {!fadeAnimation ? (
          ...
          <div
            className={`row ${fadeAnimation ? "showNav" : "hideNav"}`}
            id="navbarNav"
          >
            <ul className="navbar-nav w-100 text-end">{renderLinks()}</ul>
          </div>
        </div>
      </nav>
    </header>
